...
fn compare_and_swap<T, F>(x: &mut [T], forward: bool, compartor: &F)
where
    F: Fn(&T, &T) -> Ordering,
{
    ...
    for i in 0..mid_point {
        if compartor(&x[i], &x[mid_point + i]) == swap_condition {
...
mod tests {
    ...
    use crate::utils::{is_sorted_order, new_u32_vec};
    use crate::SortOrder::*;
    ...
    fn sort_students_by_name_ascending() {
        ...
        assert_eq!(
            sort_by(&mut x, &|a: &&Student, b: &&Student| a
                .last_name
                .cmp(&b.last_name)
    ...
    fn sort_u32_large() {
        {
            ...
            assert_eq!(sort(&mut x, &Ascending), Ok(()));
            assert!(is_sorted_order(&x, &Ascending));
        }
        {
            let mut x = new_u32_vec(65536);
            assert_eq!(sort(&mut x, &Descending), Ok(()));
            assert!(is_sorted_order(&x, &Descending));
        }
    }
}
